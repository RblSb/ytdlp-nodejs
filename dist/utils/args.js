...
function createArgs(options) {
    ...
    if (options.noUpdate)
        args.push('--no-update');
    if (options.updateTo)
        args.push('--update-to', options.updateTo);
    ...
    if (options.noMatchFilters)
        args.push('--no-match-filters');
    if (options.breakMatchFilters)
        args.push('--break-match-filters', options.breakMatchFilters);
    if (options.noBreakMatchFilters)
        args.push('--no-break-match-filters');
    if (options.noPlaylist)
        args.push('--no-playlist');
    if (options.yesPlaylist)
        args.push('--yes-playlist');
    if (options.ageLimit)
        args.push('--age-limit', options.ageLimit.toString());
    ...
    if (options.noBreakOnExisting)
        args.push('--no-break-on-existing');
    if (options.breakPerInput)
        args.push('--break-per-input');
    if (options.noBreakPerInput)
        args.push('--break-per-input');
    if (options.skipPlaylistAfterErrors)
        args.push('--skip-playlist-after-errors', options.skipPlaylistAfterErrors.toString());
    // Download Options
    ...
    if (options.bufferSize)
        args.push('--buffer-size', options.bufferSize);
    ...
    if (options.lazyPlaylist)
        args.push('--lazy-playlist');
    if (options.noLazyPlaylist)
    ...
    if (options.batchFile)
        args.push('--batch-file', options.batchFile);
    ...
    if (options.noWriteThumbnails)
        args.push('--no-write-thumbnails');
    ...
    if (options.printToFile)
        args.push('--print-to-file', options.printToFile);
    if (options.dumpJson)
        args.push('--dump-json');
    ...
    if (options.twoFactor)
        args.push('--twofactor', options.twoFactor);
    if (options.netrc)
        args.push('--netrc');
    if (options.videoPassword)
        args.push('--video-password', options.videoPassword);
    if (options.apMso)
        args.push('--ap-mso', options.apMso);
    if (options.apUsername)
        args.push('--ap-username', options.apUsername);
    if (options.apPassword)
        args.push('--ap-password', options.apPassword);
    if (options.netrcLocation)
        args.push('--netrc-location', options.netrcLocation);
    if (options.netrcCmd)
        args.push('--netrc-cmd', options.netrcCmd);
    ...
    if (options.clientCertificatePassword)
        args.push('--client-certificate-password', options.clientCertificatePassword);
    ...
    if (options.matchTitle)
        args.push('--match-title', options.matchTitle);
    if (options.rejectTitle)
        args.push('--reject-title', options.rejectTitle);
    if (options.includeAds)
        args.push('--include-ads');
    ...
    if (options.writeWeblocLink)
        args.push('--write-webloc-link');
    if (options.writeLnkLink)
        args.push('--write-lnk-link');
    // Workarounds
    ...
    if (options.replaceInMetadata) {
        for (const [key, [search, replace]] of Object.entries(options.replaceInMetadata)) {
