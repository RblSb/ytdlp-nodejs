...
class YtDlp {
    ...
    constructor(opt) {
        ...
        if (process.platform !== 'win32') {
            try {
                ...
                console.error(new Error(`Failed to set executable permissions: ${error instanceof Error ? error.message : 'Unknown error'}`));
            }
        }
    }
    // done
    ...
    async _executeAsync(args, onData, passThrough) {
        return new Promise((resolve, reject) => {
            if (!this.binaryPath)
                reject(new Error('Ytdlp binary not found'));
            const ytDlpProcess = (0, child_process_1.spawn)(this.binaryPath, args);
            let stdoutData = '';
            let stderrData = '';
            ytDlpProcess.stdout.on('data', (data) => {
